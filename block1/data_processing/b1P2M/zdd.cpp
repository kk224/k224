...
zddpoly_t *zddpoly_copy(zddpoly_t *oldzddp) //r
{
    ...
    for (var_subscript_t i = 0; i < ZDD_INPUT_VARNUM; i++)
        zddp->old_to_new->values[i] = oldzddp->old_to_new->values[i];
    
    
    zddp->new_to_old = mzp_init(ZDD_INPUT_VARNUM);
    for (var_subscript_t i = 0; i < ZDD_INPUT_VARNUM; i++)
        zddp->new_to_old->values[i] = oldzddp->new_to_old->values[i];
...
void zddpoly_free(zddpoly_t *zddp, flag_t flag) //r
{
    ...
    mzp_free(zddp->old_to_new);
    zddp->old_to_new = NULL;
    mzp_free(zddp->new_to_old);
 ...
 * p is not null
 */
void zdd_print(DdManager *zdd, DdNode *p, mzp_t *old_to_new) //r
{
    CHECK_NN(p)
    CHECK_NN(old_to_new)
    ...
    if (p == zero)
        ...
        printf("x%d",  old_to_new->values[p->index]);
        if (t != one) {
            printf("*");
            if (cuddE(t) == zero) {
...
void zddpoly_rename(zddpoly_t *zddp) //r
{
    ...
    for (var_subscript_t i = 0; i < ZDD_INPUT_VARNUM; i++) {
        ...
        if (vars[i]) {
            ...
            zddp->old_to_new->values[i] = zddp->appeared_vars;
            zddp->new_to_old->values[(zddp->appeared_vars)++] = i;
